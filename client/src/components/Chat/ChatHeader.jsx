...
function ChatHeader() {
  ...
  const showContextMenu = (e) => {
    e.preventDefault();
    setContextMenuCoordinates({ x: e.pageX - 50, y: e.pageY });
  ...
  const handleVoiceCall = () => { 
    dispatch({
      type: reducerCases.SET_VOICE_CALL, voiceCall: {
        ...currentChatUser,
        type: "out-going",
        callType: "voice",
        roomId: Date.now()
  ...
  const handleVideoCall = () => { 
    dispatch({
      type: reducerCases.SET_VIDEO_CALL, videoCall: {
        ...currentChatUser,
        type: "out-going",
        callType: "video",
        roomId: Date.now()
  ...
  return <div className=" h-16 px-4 py-3 flex justify-between items-center bg-panel-header-background z-10 ">
    ...
    <div className="flex gap-6">
      ...
      <BsThreeDotsVertical onClick={e => showContextMenu(e)} className=" text-panel-header-icon cursor-pointer text-xl" id="context-opener" />
      {
